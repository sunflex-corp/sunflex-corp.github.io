...
const {pose}=require('../assets/sunflex-v2/product-motion.js');
...
console.log('PASS: clamped image scale/depth, reversible pose, mobile amplitude');
...
const root={dataset:{motionProfile:'site'},querySelectorAll(selector){if(selector.includes('[data-composition=panorama]'))return [picture];if(selector.includes('.editorial-hero-copy'))return [heading];return [];}};
