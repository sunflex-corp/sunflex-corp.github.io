...
def render(path, title, desc, body, active=''):
    ...
    if path == 'index.html':
        for ext in ['css','js']:
            version=hashlib.sha256((ROOT/f'assets/sunflex-v2/home-motion.{ext}').read_bytes()).hexdigest()[:10]
...
def infographic():
    options=[('영상 확인','카메라로 작업 구역을 확인하세요.','시야 확보가 어려운 구역과 이동하는 작업 현장에 맞는 영상장비를 살펴보세요.','/products/mobile-cctv/','이동식 CCTV 보기'),('위험 알림','접근과 위험을 알리는 장비.','차량과 작업자가 함께 이동하는 구역에서 경보와 시각 안내에 필요한 제품을 확인하세요.','/products/equipment-approach-alarm/','접근경보 시스템 보기'),('환경 측정','눈에 보이지 않는 작업 환경까지.','가스 농도와 작업 환경을 확인하는 측정·경보 장비를 살펴보세요.','/products/compact-gas-detector/','복합가스 측정기 보기')]
    buttons=''.join(f'<button type="button" class="map-choice" data-map-choice="{i}" aria-pressed="{str(i==0).lower()}" aria-controls="map-panel-{i}"><span class="map-choice-dot" aria-hidden="true"></span>{name}<span aria-hidden="true">↗</span></button>' for i,(name,*_) in enumerate(options))
    panels=''.join(f'<div class="map-panel" id="map-panel-{i}"{ " hidden" if i else ""}><h3>{title}</h3><p>{desc}</p>{link(href,label)}</div>' for i,(_,title,desc,href,label) in enumerate(options))
    return f'''<section class="section safety-map" id="safety-map"><div class="wrap"><div class="lead"><div><span class="eyebrow">현장 안전, 기능별로 살펴보기</span><h2>영상 확인부터<br>작업 환경 측정까지.</h2></div><p>영상 확인부터 위험 알림, 작업 환경 측정까지.<br>필요한 기능을 선택해 관련 제품을 확인하세요.</p></div><div class="map-layout"><div class="map-visual" data-mode="0"><span class="map-kicker">SUNPLEX / SAFETY SYSTEMS</span><button type="button" class="map-motion-toggle" aria-pressed="false" hidden>그래픽 정지</button><svg class="safety-diagram" viewBox="0 0 720 520" role="img" aria-labelledby="map-title map-desc"><title id="map-title">산업현장 안전장비 구성 개념도</title><desc id="map-desc">중앙 작업 구역 주변에 영상 확인, 위험 알림, 환경 측정 장비를 배치한 개념도입니다.</desc><defs><linearGradient id="map-floor" x2="1" y2="1"><stop stop-color="#243753"/><stop offset="1" stop-color="#0c1421"/></linearGradient><radialGradient id="map-glow"><stop stop-color="#87afff" stop-opacity=".25"/><stop offset="1" stop-color="#87afff" stop-opacity="0"/></radialGradient></defs><ellipse cx="360" cy="300" rx="290" ry="210" fill="url(#map-glow)"/><g class="map-orbits" fill="none" stroke="#344863"><ellipse cx="360" cy="305" rx="300" ry="157"/><ellipse cx="360" cy="305" rx="248" ry="130" stroke-dasharray="3 11"/></g><path d="M110 290 360 150 610 290 360 438Z" fill="url(#map-floor)" stroke="#637b9b"/><g stroke="#31445f" fill="none"><path d="m160 318 250-140m-200 168 250-140m-200 168 250-140m-200 168 250-140M160 262l250 148M210 234l250 148M260 206l250 148M310 178l250 148"/></g><g class="warehouse"><path d="M286 276V193L366 151 458 203V286L378 331Z" fill="#162638" stroke="#7994b5"/><path d="m286 193 92 53 80-43M378 246v85" fill="none" stroke="#b0c8e5"/><path d="m303 205 58-31 79 45M398 255v43l37-21v-43Z" fill="#253e59" stroke="#7594b5"/><path d="m304 233 50 28m-50-12 50 28m-50-12 50 28" stroke="#7b99bf"/></g><g class="map-route route-0" fill="none" stroke="#91b6ff" stroke-width="2"><path d="M172 185 244 226 285 251"/><path class="data-flow" d="M172 185 244 226 285 251"/></g><g class="map-route route-1" fill="none" stroke="#ffba73" stroke-width="2"><path d="M548 230 480 269 458 280"/><path class="data-flow" d="M548 230 480 269 458 280"/></g><g class="map-route route-2" fill="none" stroke="#82ddce" stroke-width="2"><path d="M341 418 341 364 378 331"/><path class="data-flow" d="M341 418 341 364 378 331"/></g><g class="map-node node-0" transform="translate(142 130)"><circle class="node-halo" cx="30" cy="38" r="51"/><rect width="60" height="60" rx="16"/><path d="M14 22h25v20H14zm25 6 10-6v20l-10-6"/><text x="30" y="94">영상 확인</text></g><g class="map-node node-1" transform="translate(518 178)"><circle class="node-halo" cx="30" cy="38" r="51"/><rect width="60" height="60" rx="16"/><path d="M17 41h26l-4-8V24a9 9 0 0 0-18 0v9zm9 7h8M11 24a19 19 0 0 1 5-12m33 12a19 19 0 0 0-5-12"/><text x="30" y="94">위험 알림</text></g><g class="map-node node-2" transform="translate(311 382)"><circle class="node-halo" cx="30" cy="30" r="51"/><rect width="60" height="60" rx="16"/><path d="M27 17v20a8 8 0 1 0 7 0V17a3.5 3.5 0 0 0-7 0m15 5h6m-6 8h6"/><text x="30" y="94">환경 측정</text></g></svg><span class="map-note">현장별 적용을 설명하기 위한 개념도</span></div><div class="map-content"><noscript><p>영상·경보·측정 제품을 <a href="/products/">전체 제품 목록</a>에서 확인하세요.</p></noscript><div class="map-choices" role="group" aria-label="안전 기능 선택">{buttons}</div><div class="map-panels" aria-live="polite">{panels}</div><p class="map-footnote">장비 구성과 연동 여부는 제품 사양 및 설치 환경에 따라 확인이 필요합니다.</p></div></div></div></section>'''
...
def home():
    ...
    scenes=[('field-crane-hook-rigging-generated','크레인 인양 장비를 점검하는 작업 장면','건설·토목','시야가 닿기 어려운 곳까지','인양 구역과 작업 동선을 영상으로 확인하세요.'),('field-vehicle-pedestrian-generated','차량과 작업자가 함께 이동하는 산업현장','제조·물류','차량과 사람이 만나는 현장','이동 구역에 필요한 접근 경보와 안내 장비를 살펴보세요.'),('company-promise-night-worker-generated','조명이 켜진 산업현장을 살펴보는 작업자','현장 운영','현장의 변화를 확인하는 기술','영상과 측정 정보를 활용해 현장 관리에 필요한 정보를 확인하세요.')]
    slides=''.join(f'<div class="scene{ " is-active" if i==0 else ""}" data-scene="{i}" aria-hidden="{str(i!=0).lower()}">{picture(img,alt,i==0,"100vw")}</div>' for i,(img,alt,*_) in enumerate(scenes))
    selectors=''.join(f'<button type="button" class="scene-select" data-select="{i}" aria-pressed="{str(i==0).lower()}" aria-label="{i+1}번 장면: {label}"><span class="scene-number">0{i+1}</span><span>{label}</span><span class="scene-track" aria-hidden="true"><i></i></span></button>' for i,(_,_,label,*_) in enumerate(scenes))
    body=f'''<section class="hero cinema" aria-label="산업현장 소개" aria-roledescription="슬라이드 쇼"><div class="scene-stage">{slides}</div><div class="cinema-grid" aria-hidden="true"></div><div class="wrap hero-copy"><span class="eyebrow"><i class="signal-dot" aria-hidden="true"></i> SUNPLEX · 산업안전 솔루션</span><h1>산업현장을 위한<br><span>스마트 안전 솔루션.</span></h1><p class="intro">영상관제, 위험 알림, 작업 환경 측정까지.<br>현장에 필요한 안전장비를 확인하세요.</p><div class="actions">{link('/solutions/','적용 분야 보기','btn')}{link('/products/','제품 전체 보기')}</div></div><div class="wrap cinema-bottom"><div class="scene-caption" aria-live="off"><span class="scene-topic">건설·토목</span><h2 class="scene-title">시야가 닿기 어려운 곳까지</h2><p class="scene-description">인양 구역과 작업 동선을 영상으로 확인하세요.</p></div><div class="scene-controls" hidden><div class="scene-selectors">{selectors}</div><div class="scene-arrows"><button type="button" data-prev aria-label="이전 장면">←</button><button type="button" data-next aria-label="다음 장면">→</button><button type="button" data-play aria-label="자동 전환 일시정지">일시정지</button></div></div></div><p class="sr-only" id="scene-status" role="status"></p></section>
<div class="wrap"><div class="hero-bottom"><span>건설 · 제조 · 물류 현장</span><a href="#safety-map">안전 솔루션 살펴보기 <span aria-hidden="true">↓</span></a></div></div>
{infographic()}
