...
function setup(reduce=false){
 ...
 const nodes={};let writes=0;
 const mapping={'.solar-cutout':{transform:'--word-scale'},'.solar-white':{opacity:'--white-opacity'},'.solar-aperture':{opacity:'--mask-opacity'},'.solar-backdrop':{transform:'--photo-scale'},'.solar-story-copy':{opacity:'--copy-opacity'},'.solar-story-hint':{opacity:'--hint-opacity'}};
 for(const name of [...Object.keys(mapping),'.solar-shade']) nodes[name]={style:new Proxy({}, {set(obj,k,v){writes++;obj[k]=v;const prop=mapping[name]?.[k];if(prop)props[prop]=v.startsWith('scale(')?+v.slice(6,-1):+v;return true;}})};
 const copy=nodes['.solar-story-copy'],stage={offsetHeight:800,classList:cl(new Set())},story={offsetHeight:2720,classList:cl(storyClasses),getBoundingClientRect:()=>({top:800-win.scrollY}),querySelector:s=>nodes[s]||stage};
...
const before=t.writes;t.scroll(0);assert.equal(t.writes,before,'unchanged/offscreen progress must not rewrite styles');
