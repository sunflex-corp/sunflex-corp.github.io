...
def matcher(cw):
 ...
 for name in ['product-infographics.css','product-infographics-layout.css']:
  nodes=tinycss2.parse_stylesheet((ROOT/'assets/sunflex-v2'/name).read_text(),skip_whitespace=True,skip_comments=True)
...
for width in [360,540,720]:
 ...
 for slug,figures in DATA.items():
  ...
  for n in root.iter_subtree():
   ...
   if 'signal-route' in c and width<=600:assert style['grid-template-columns']=='minmax(0,1fr)',(slug,style)
 ...
 assert large==16 and renders==31 and panels==144 and pins==15,(width,large,renders,panels)
 count+=panels
print(f'PASS: {count} scene/width cascade checks; 16 full-size SVG scenes, 31 render frames, readable compact card/route rules')
