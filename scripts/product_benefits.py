...
def build(slug, name, kind, soup):
    ...
    for track in list(soup.select('[data-scroll-flow]')):
        if slug in ['mobile-cctv','site-cms']:continue
    ...
    for i,s in enumerate(stages):
        ...
        if slug=='site-cms':
            old=soup.select('#process-transition-line .editorial-flow img')
            if i<len(old):
        ...
        # A source product shot is contained; a contextual photograph fills the scene.
        fit=s.get('fit',asset['fit']);position=asset['position']
        figure=f'<figure class="benefit-photo" data-fit="{esc(fit)}"><img src="{esc(asset["src"])}" srcset="{esc(asset["srcset"])}" sizes="(max-width:760px) 90vw, 650px" width="{asset["width"]}" height="{asset["height"]}" alt="{esc(asset["alt"])}" loading="lazy" decoding="async" style="object-fit:{esc(fit)};object-position:{esc(position)}"></figure>'
