(() => {
  ...
  if (!hero) return;
  const reduced = new URLSearchParams(location.search).get('motion') === 'reduce' ? {matches:true,addEventListener(){}} : window.matchMedia('(prefers-reduced-motion: reduce)');
  ...
  const updatePlayback = () => {
    stop();
    hero.dataset.sunPaused = String(paused);
